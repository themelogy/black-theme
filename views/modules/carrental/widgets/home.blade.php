<div class="container">
    ...
    <div class="owl-carousel" id="owl-carousel" data-items="4" data-items-mobile="1" data-items-tablet="2" data-dots="false">
        ...
        <div>
            <div class="thumb">
                <header class="thumb-header" style="min-height: 170px;">

                    <h5 class="thumb-title"><a class="text-darken" href="{{ $car->reservationUrl }}">{!! Html::image($car->brand->present()->firstImage(20,null,'resize',50), $car->brand->name, ['style'=>'width:20px;', 'class'=>'lazyloader']) !!}  {{ $car->fullname }}</a></h5>
                    <a href="{{ $car->url }}">
                        <img class="lazyloader img-responsive" src="{{ $car->present()->firstImage(null,160,'resize',50) }}" alt="{{ $car->fullname }}" title="{{ $car->fullname }}" />
                    </a>
                </header>
